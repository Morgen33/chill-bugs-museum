  ...
  return (
    <main className="fixed inset-0 overflow-hidden bg-black text-fg">
      ...
      <button
        type="button"
        onClick={toggleSound}
        className="absolute top-4 right-4 z-20 rounded-full border border-white/20 bg-black/55 px-3 py-2 text-[11px] font-semibold tracking-[0.18em] text-white/90 uppercase backdrop-blur-sm transition hover:border-white/45 hover:bg-black/75 sm:top-6 sm:right-6"
        aria-label={muted ? "Unmute video" : "Mute video"}
      >
        {muted ? "Sound on" : "Sound off"}
      </button>

      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 flex flex-col items-center gap-4 bg-gradient-to-t from-black/55 to-transparent px-5 pt-20 pb-8 sm:gap-5">
        <Image
          src="/logo.png"
          alt="Chill Bugs"
          width={512}
          height={512}
          priority
          className="pointer-events-auto h-28 w-28 object-contain sm:h-36 sm:w-36"
        />

        <nav
          aria-label="Social"
          className="pointer-events-auto flex items-center justify-center gap-8 sm:gap-10"
        >
          {SOCIAL_LINKS.map((link) => (
            <a
              ...
              className="text-white/85 drop-shadow-md transition hover:text-white"
            >
        ...
        <Link
          href="/museum"
          className="pointer-events-auto font-serif text-2xl tracking-[0.2em] text-white/90 uppercase drop-shadow-md transition hover:text-white sm:text-3xl"
        >
          Museum
        </Link>
      </div>
