...
function openseaHeaders(apiKey: string): HeadersInit {
  return {
...
export async function fetchBugByTokenId(
  tokenId: string,
): Promise<ChillBug | null> {
  ...
  const res = await fetch(
    `https://api.opensea.io/api/v2/chain/ethereum/contract/${CHILLBUGS_CONTRACT}/nfts/${tokenId}`,
    {
      headers: openseaHeaders(apiKey),
      next: { revalidate: 300 },
    },
  ...
  return mapNft(json.nft, collectionTokensScored);
}
